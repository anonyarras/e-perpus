<div class="container-fluid py-4">
    <div class="panel-heading">
        <div class="col-md-6">Tambah Buku</div>
        <div class="col-md-6">
            <a href="{{route('buku')}}" class="btn btn-success pull-right">Kembali</a>
                    ...
                    @if(Session::has('message'))
                        ...
                        @endif
    <form wire:submit.prevent='addBuku'>
        <div class="form-group">
            <label for="example-text-input" class="form-control-label">Judul Buku</label>
            <input class="form-control" wire:model='judul' wire:keyup='generateslug' type="text" placeholder="Pengetahuan Alam" id="example-text-input">
            @error('judul')
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="file" wire:model='image' id="example-text-input">
            @if ($image)
                <img src="{{$image->temporaryUrl()}}" width=120 alt="" class="">
            @endif
            @error('image')
            <p class="text-danger">{{$message}} </p>
            @enderror
